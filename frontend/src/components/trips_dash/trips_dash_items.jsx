...
import { tripStartDateFinder, tripEndDateFinder, formatDate } from '../../utils/datetime_api_util';
  ...
  const tripButtons = trips.map((trip, idx) => {
    ...
    const tripDates = trip.destinations.length > 0 ? 
      <div className="date-info">
        <h4>{`${formatDate(tripStartDateFinder(trip.destinations))}`}</h4>
        <h4>to</h4>
        <h4>{`${formatDate(tripEndDateFinder(trip.destinations))}`}</h4>
      </div>
     : "";
